...
```{r}
 county_MI %>% mutate(tot_job_type = Professional + Service + Office + Construction + Production) %>%
 select(County, tot_job_type)
...
county_MI %>%
  group_by(peninsula) %>%
  summarise(num_counties = n_distinct(County))
```
...
ddply(county_MI, .(peninsula), summarise, num_counties = n_distinct(County))
```
...
summarise(mean_Income = mean(Income), 
          ...
          trim_Mean_Income = mean(Income, trim = 0.05),
          coefficient_variation = sd(Income)/mean(Income),
          IQR_Income = IQR(Income),
          quantile_Income_0.05 = quantile(Income, 0.05),
          quantile_Income_0.25 = quantile(Income, 0.25),
          quantile_Income_0.50 = quantile(Income, 0.50),
          quantile_Income_0.75 = quantile(Income, 0.75),
          quantile_Income_0.95 = quantile(Income, 0.95)) 
...
county_df %>%
  group_by(State) %>%
  summarise( quantile_Income_0.05 = quantile(Income, 0.05, na.rm = TRUE),
     quantile_Income_0.50 = quantile(Income, 0.50, na.rm = TRUE),
      quantile_Income_0.95 = quantile(Income, 0.95, na.rm = TRUE)) %>%
  arrange(desc(quantile_Income_0.05, quantile_Income_0.50, quantile_Income_0.95))
```
...
```{r correlation}
cor_matrix <- county_df[c(7, 19, 22)]
...
**All of the variables will be 100% correlated with themselves, so one can ingore those relationships. TotalPop is positively correlated with IncomePerCap, but weakly so, and is negatively correlated with Child Poverty, again insignificantly so. A strong downhill relationsihp does exist between IncomePerCap and ChildPoverty, which is intuitive, since poverty would naturally decline as Income level increased.**
...
select(County, SelfEmployed) %>%
   ggplot( aes(SelfEmployed) ) + geom_histogram() +labs(title = "Count of counties with pop > 75000 by % self employed", x = "Percentage self employed", y = "count")
```
...
```{r create_df_toplot}
 pop_by_region <- county_df %>%
   group_by(Region) %>%
   summarise(Total_Pop_By_Region = sum(TotalPop))
pop_by_region   

```

```{r barchart}
 ggplot(data=pop_by_region, aes(pop_by_region$Region, pop_by_region$Total_Pop_By_Region)) + geom_bar(stat = "identity") + labs(title = "Total pop by region", x = "Region", y = "Total pop")
```
...
```{r}
 ...
 ggplot(data=pop_by_region, aes(x = reorder(pop_by_region$Region, pop_by_region$Total_Pop_By_Region) , y = pop_by_region$Total_Pop_By_Region)) + geom_bar(stat = "identity") + coord_flip() + labs(title = "Total pop by region", x = "Region", y = "Total Pop")  
